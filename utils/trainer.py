...
class Trainer(object):
    ...
    def val_time_ensemble(self, model, test_data_loader, accelerator, thresholding=False, save_to=None):
        ...
        def cal_miou(gt, res, thresholding, save_to=None, n=None):
            res = res.cpu().numpy().squeeze()
            if save_to is not None:
                plt.imsave(os.path.join(save_to, n), res, cmap='gray')
            gt_binary = (gt > 0.5).astype(np.uint8)
            res_binary = (res > 0.5).astype(np.uint8)
            intersection = np.logical_and(gt_binary, res_binary)
            union = np.logical_or(gt_binary, res_binary)
            iou = np.sum(intersection) * 1.0 / (np.sum(union) + 1e-8)
            return np.sum(intersection) * 1.0 / (np.sum(union) + 1e-8)
        ...
        for data in tqdm(test_data_loader, disable=not accelerator.is_main_process):
            ...
            ensemble_mious += [cal_miou(g, r, thresholding, save_to, n) for g, r, n in zip(gt, ensem_res, name)]

        # gather all the results from different processes
        accelerator.wait_for_everyone()
        ensemble_maes = accelerator.gather(torch.tensor(ensemble_maes).mean().to(device)).mean().item()
        ensemble_mious = accelerator.gather(torch.tensor(ensemble_mious).mean().to(device)).mean().item()
        _best_mae = min(_best_mae, ensemble_maes)
        _best_miou = max(_best_miou, ensemble_mious)
        print(ensemble_mious, _best_miou)
        return ensemble_mious, _best_miou
    ...
    def train(self):
        ...
        for epoch in range(self.cur_epoch, self.train_num_epoch):
            ...
            if (epoch + 1) % 1 == 0 or (epoch >= self.train_num_epoch * 0.7):
                miou, best_miou = self.val_time_ensemble(self.model, self.test_loader, accelerator)
                self.logger.info(f'Epoch:{epoch}/{self.train_num_epoch} miou: {miou:.4f}({best_miou:.4f})')
                accelerator.log({'miou': miou, 'best_miou': best_miou})
                if miou == best_miou:
                    self.save("best")
